...
fn get_val(mem: &mut FxHashMap<usize, usize>, initial_turn: usize, final_turn: usize, initial_val: usize) -> usize {
...
fn get_initial_numbers(mut f: BufReader<File>) -> FxHashMap<usize, usize> {
    let mut s = String::new();
    let _ = f.read_line(&mut s);
    let split: Vec<&str> = s.split(',').collect();

    let state = split.iter()
                 .enumerate()
                 .map(|(i, spl)| (spl.parse().unwrap(), i + 1))
                 .collect();
